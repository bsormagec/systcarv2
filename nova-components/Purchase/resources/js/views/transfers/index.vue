    ...
    <!-- Search -->
        ...
        <div class="pr-2 w-2/5">
          <select v-model="queryFiled" class="w-full form-control form-select" id="fileds">
            <option value="nit">CI\NIT</option>
            <option value="invoice_number">Nro Factura</option>
          ...
          >
            <thead>
              <tr>
                ...
                <th class="text-center"><span>Nº Factura</span></th>
                <th class="text-center"><span>Total</span></th>
                <th class="text-center"><span>Descuento</span></th>
            ...
            <tbody>
              <tr v-for="item in compras" :key="item.id">
                <td class="text-center">{{ item.proveedor.business_name }}</td>
                <td class="text-center">{{ item.fecha }}</td>
                <td class="text-center">{{ item.nrofactura }}</td>
                  ...
                  <div class="inline-flex items-center">
                     <!-- Delete Resource Link -->
                        <button
                            ...
                            class="inline-flex appearance-none cursor-pointer text-70 hover:text-primary mr-3"
                            v-tooltip.click="__('Anular Compra')"
...
export default {
  components:{
    ...
    beforeRouteEnter(to, from, next) {
     get('/nova-vendor/purchase/purchases', to.query)
          .then((res) => {
    ...
    methods: {
      getData() {
          this.loading = true;
          get("/nova-vendor/purchase/purchases?page=" + this.pagination.current_page)
            .then(response => {
      ...
      setData(res) {
          Vue.set(this.$data, 'compras', res.data.data)
       ...
       searchData() {
          this.loading = true;
          get(
              "/nova-vendor/purchase/search/purchases/" +
                this.queryFiled +
                "/" +
                this.search +
                "?page=" +
      ...
      detailsPage(item) {
          ...
          this.$router.push({ name: 'showpurchase', params: { id } })
        },
         nextPage() {
      ...
      openDeleteModal() {
        ...
        confirmDelete(id) {
          byMethod('delete', `/nova-vendor/purchase/purchases/${id}`)
                    .then((res) => {
                      console.log(res.data)
                      if(res.data.deleted) {
                        this.$toasted.show('Compra Anulada con exito', { type: 'success' });
                        this.getData();
                      }
                    })
