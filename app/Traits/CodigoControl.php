...
use App\Traits\AllegedRC4;
use App\Traits\Verhoeff;
use App\Traits\Base64SIN;
/*
 * traits para generar el codigo de control de la factura 
 */
trait CodigoControl
{
    use AllegedRC4,Base64SIN,Verhoeff;
    
    public static function generar($authorizationNumber, $invoiceNumber, $nitci, $dateOfTransaction, $transactionAmount, $dosageKey)
	{
       //validación de datos
       if( empty($authorizationNumber) || empty($invoiceNumber) || empty($dateOfTransaction) ||
            ...
            self::validateNumber($authorizationNumber);
            self::validateNumber($invoiceNumber);
            self::validateNumber($dateOfTransaction);
            self::validateNumber($nitci);
            self::validateNumber($transactionAmount);
            self::validateDosageKey($dosageKey);
        }

        //redondea monto de transaccion
        $transactionAmount = self::roundUp($transactionAmount);
    ...
    static function validateDosageKey($value){
